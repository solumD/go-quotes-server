...
func TestGetAllQuotes(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:               "error failed to get all quotes",
			mockError:          errors.New("srv error"),
			expectedError:      errors.New("failed to get all quotes"),
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			mockService.EXPECT().GetAllQuotes(gomock.Any()).Return([]*model.Quote{}, tc.mockError)

			h := handler.New(mockService).GetAllQuotes(context.Background(), sl.NewDiscardLogger())

			req, _ := http.NewRequest(http.MethodGet, fmt.Sprintf("/quotes"), nil)
